...
SAMPLE_FRACTION = 1  # Cambiar este valor para reducir el volumen de datos
...
y_train = data_jl['y_train']
y_test = data_jl['y_test']
...
def showResults(y_test, y_pred, model_name, reduction_method, y_pred_proba=None):
    print(f"\n{'='*60}")
    ...
    # Calcular AUROC si se proporcionan probabilidades
    auroc = None
...
for method_name, (X_train_method, X_test_method) in reduction_methods.items():
    ...
    y_pred_proba = rf_model.predict_proba(X_test_method)[:, 1]  # Probabilidad de la clase positiva
    
    # Mostrar y guardar resultados
    results = showResults(y_test, y_pred, 'random_forest', method_name, y_pred_proba)
